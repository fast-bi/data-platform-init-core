-- Removes tables and views from the given run configuration for Microsoft Fabric (Synapse)
-- Usage in production:
            ...
            {% for database in current_model_locations.keys() %}
                ...
                {% for schema, tables in current_model_locations[database].items() %}
                    ...
                    select
                        ...
                        concat(quote_ident(table_catalog), '.', quote_ident(table_schema), '.', quote_ident(table_name)) as relation_name,
                        concat(quote_ident(table_catalog), '.', quote_ident(table_schema)) as schema_relation_name
