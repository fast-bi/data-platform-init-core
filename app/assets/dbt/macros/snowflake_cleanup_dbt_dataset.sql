-- Removes tables and views from the given run configuration for Snowflake
-- Usage in production:
            ...
            {% for database in current_model_locations.keys() %}
                ...
                {% for schema, tables in current_model_locations[database].items() %}
                    ...
                    select
                        ...
                        database_name || '.' || schema_name || '.' || table_name as relation_name,
                        database_name || '.' || schema_name as schema_relation_name
